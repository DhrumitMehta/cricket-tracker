  ...
  const captureFrame = async () => {
    ...
    try {
      ...
      if (!videoPlayerRef.current) {
        throw new Error('Video player view not found');
      }

      // Add a small delay to ensure the view is ready
      await new Promise(resolve => setTimeout(resolve, 100));

      // Capture the current frame using react-native-view-shot
      const uri = await captureRef(videoPlayerRef.current, {
        format: 'jpg',
        quality: 1,
        result: 'tmpfile',
      });
      ...
      // Move the captured frame to our frames directory

...
type AnnotationAction = ImageManipulator.Action & {
  drawLines?: {
    lines: Array<{
      start: { x: number; y: number };
      end: { x: number; y: number };
...
const Analysis = () => {
  ...
  const convertToAnnotation = (videoAnnotation: VideoAnnotation): Annotation | null => {
    if (videoAnnotation.type === 'text' && videoAnnotation.text && videoAnnotation.points[0]) {
      return {
        text: {
          text: videoAnnotation.text,
          position: videoAnnotation.points[0],
          color: '#ff0000',
          fontSize: 20
        }
      };
    } else if (videoAnnotation.type === 'line' && videoAnnotation.points.length > 1) {
      return {
        draw: {
          lines: videoAnnotation.points.slice(1).map((point, i) => ({
            start: videoAnnotation.points[i],
            end: point,
            color: '#ff0000',
            thickness: 2
          }))
  ...
  const drawAnnotationsOnFrame = async (
    ...
    const frameAnnotations = annotations
      .filter(a => Math.abs(a.timestamp - frameTime) < 0.1)
    ...
    for (const annotation of frameAnnotations) {
      if ('text' in annotation) {
        actions.push({
          resize: {
            width: annotation.text.position.x,
            height: annotation.text.position.y
          }
        });
      } else if ('draw' in annotation && annotation.draw.lines.length > 0) {
        for (const line of annotation.draw.lines) {
          actions.push({
            crop: {

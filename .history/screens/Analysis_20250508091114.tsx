...
const Analysis = () => {
  ...
  const captureFrame = async () => {
    ...
    try {
      ...
      // Use react-native-view-shot with a small delay to ensure the frame is ready
      await new Promise(resolve => setTimeout(resolve, 100));
      
      const screenshotUri = await captureRef(videoPlayerRef.current, {
        format: 'jpg',
        quality: 1,
      });

      // Use image manipulator to process the captured frame
      const processedImage = await manipulateAsync(
        screenshotUri,
        [{ resize: { width: 1280 } }], // Resize to a reasonable width while maintaining aspect ratio
        { compress: 0.9, format: SaveFormat.JPEG }
      );

      // Move the processed image to our frames directory
      ...
      Alert.alert('Success', 'Frame captured successfully');
    } catch (error) {
      console.error('Frame capture error:', error);
      Alert.alert('Error', error instanceof Error ? error.message : 'Failed to capture frame');
    } finally {
  ...
  // Add FFmpeg processing function
  const processVideo = async (videoUri: string) => {
    try {
      setIsProcessing(true);
      
      // Example FFmpeg command for video processing
      const command = `-i ${videoUri} -c:v mpeg4 -preset ultrafast ${videoUri}_processed.mp4`;
      ...
      if (ReturnCode.isSuccess(returnCode)) {
        ...
        Alert.alert('Cancelled', 'Processing was cancelled');
      } else {
            ...
            {videoSource && (
              <View style={styles.videoContainer} ref={videoPlayerRef}>
                ...
                <View style={styles.toolbarContainer}>
                  ...
                  <Button
                    mode="outlined"
                    onPress={() => processVideo(videoSource)}
                    loading={isProcessing}
                    disabled={isProcessing}
                    style={styles.toolButton}
                  >

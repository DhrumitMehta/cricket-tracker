    ...
    <View style={styles.loopControlsContainer}>
      ...
      <View style={styles.loopControlsRow}>
        <View style={styles.timeInputContainer}>
          <Text>Start: {formatTime(loopStartTime)}</Text>
          <TouchableOpacity
            style={styles.setTimeButton}
            onPress={() => onRangeChange(currentTime, loopEndTime || duration)}
          >
            <Text style={styles.buttonText}>Set Start</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.timeInputContainer}>
          <Text>End: {formatTime(loopEndTime || duration)}</Text>
          <TouchableOpacity
            style={styles.setTimeButton}
            onPress={() => onRangeChange(loopStartTime, currentTime)}
          >
        ...
        <TouchableOpacity
          ...
          onPress={() => onRangeChange(0, duration)}
        >
    ...
    try {
      if (start === 0 && end === videoDuration) {
        ...
        if (primaryVideoRef.current) {
          ...
          // If we're setting the start time and we're past it, or we're before it, seek to it
      ...
      console.error('Error updating loop range:', error);
    }
  };
